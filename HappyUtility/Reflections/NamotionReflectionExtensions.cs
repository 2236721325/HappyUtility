using HappyUtility.Reflections;
...
namespace HappyUtility.Reflections
{
    public static class NamotionReflectionExtensions
    {
        public static string ToCodeStringIgnoreNullable(this ContextualType @this)
        {
            if (@this.GenericArguments.Length > 0)
            {
                var innerCode = string.Join(',', values: @this.GenericArguments.Select(e => e.ToCodeString()));
